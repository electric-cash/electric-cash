...
env | grep -E '^(ELCASH_CONFIG|BASE_|QEMU_|CCACHE_|WINEDEBUG|LC_ALL|BOOST_TEST_RANDOM|CONFIG_SHELL|(ASAN|LSAN|TSAN|UBSAN)_OPTIONS|TEST_PREVIOUS_RELEASES|PREVIOUS_RELEASES_DIR)' | tee /tmp/env
if [[ $HOST = *-mingw32 ]]; then
  DOCKER_ADMIN="--cap-add SYS_ADMIN"
elif [[ $ELCASH_CONFIG = *--with-sanitizers=*address* ]]; then # If ran with (ASan + LSan), Docker needs access to ptrace (https://github.com/google/sanitizers/issues/764)
...
if [ ! -d ${DIR_QA_ASSETS} ]; then
 if [ "$RUN_FUZZ_TESTS" = "true" ]; then
  DOCKER_EXEC git clone https://github.com/elcash-core/qa-assets ${DIR_QA_ASSETS}
 fi
fi
