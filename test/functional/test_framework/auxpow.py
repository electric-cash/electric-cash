...
def constructAuxpow (block):
  ...
  header = b"01000000"
  header += b"00" * 32
  header += reverseHex(doubleHashHex(tx)) # hashMerkleRoot
  header += b"00" * 4
  header += b"00" * 4
  header += b"00" * 4
...
def finishAuxpow (tx, header):
  ...
  auxpow += blockhash
  auxpow += b"00" # empty vMerkleBranch
  auxpow += b"00" * 4 # nMerkleIndex
  auxpow += b"00" # empty vChainMerkleBranch
  auxpow += b"00" * 4 # nChainIndex
  auxpow += header
