...
class SegWitTest(BitcoinTestFramework):
    ...
    def fail_accept(self, node, error_msg, txid, sign, redeem_script=""):
        assert_raises_rpc_error(-26, error_msg, send_to_witness, use_p2wsh=1, node=node, utxo=getutxo(txid), pubkey=self.pubkey[0], encode_p2sh=False, amount=Decimal("499.998"), sign=sign, insert_redeem_script=redeem_script)
    ...
    def run_test(self):
        ...
        for i in range(5):
            for n in range(3):
                for v in range(2):
                    wit_ids[n][v].append(send_to_witness(v, self.nodes[0], find_spendable_utxo(self.nodes[0], 500), self.pubkey[n], False, Decimal("499.999")))
                    p2sh_ids[n][v].append(send_to_witness(v, self.nodes[0], find_spendable_utxo(self.nodes[0], 500), self.pubkey[n], True, Decimal("499.999")))
        ...
        txid1 = send_to_witness(1, self.nodes[0], find_spendable_utxo(self.nodes[0], 500), self.pubkey[0], False, Decimal("499.996"))
    ...
    def mine_and_test_listunspent(self, script_list, ismine):
        utxo = find_spendable_utxo(self.nodes[0], 500)
        tx = CTransaction()
