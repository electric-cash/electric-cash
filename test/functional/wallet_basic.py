...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(walletinfo['immature_balance'], 450)
        ...
        assert_equal(txout['value'], 450)
        txout = self.nodes[0].gettxout(txid=confirmed_txid, n=confirmed_index, include_mempool=True)
        assert_equal(txout['value'], 450)

        # Send 21 ELCASH from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 100 elcash in block rewards plus fees, but
        # minus the 21 plus fees sent to node2
        assert_equal(self.nodes[0].getbalance(), 900 - 21)
        ...
        assert_equal(self.nodes[2].getbalance(), 894)

        # Verify that a spent output cannot be locked anymore
        spent_0 = {"txid": node0utxos[0]["txid"], "vout": node0utxos[0]["vout"]}
        assert_raises_rpc_error(-8, "Invalid parameter, expected unspent output", self.nodes[0].lockunspent, False, [spent_0])

        # Send 10 ELCASH normal
        ...
        self.sync_all(self.nodes[0:3])
        node_2_bal = self.check_fee_amount(self.nodes[2].getbalance(), Decimal('884'), fee_per_byte, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(self.nodes[0].getbalance(), Decimal('10'))

        # Send 10 ELCASH with subtract fee from amount
        ...
        # Sendmany 10 ELCASH with subtract fee from amount
        ...
        usp = self.nodes[1].listunspent(query_options={'minimumAmount': '449.998'})[0]
        inputs = [{"txid": usp['txid'], "vout": usp['vout']}]
        outputs = {self.nodes[1].getnewaddress(): 449.998, self.nodes[0].getnewaddress(): 11.11}
        ...
        maintenance = [
            ...
            '-zapwallettxes=1',
            '-zapwallettxes=2',
            # disabled until issue is fixed: https://github.com/electric-cash/electric-cash/issues/7463
            # '-salvagewallet',
        ]
        chainlimit = 6
