...
BOOST_AUTO_TEST_CASE(MempoolIndexingTest)
{
    ...
    pool.addUnchecked(entry.Fee(1LL).FromTx(tx3));
    ...
    pool.addUnchecked(entry.Fee(1LL).FromTx(tx6));
    ...
    uint64_t zero = 0;
    BOOST_CHECK_EQUAL(pool.CalculateMemPoolAncestors(entry.Fee(2000000LL).FromTx(tx7), setAncestorsCalculated, 100, 1000000, 1000, 1000000, dummy, false, nNoLimit, nNoLimit, zero, zero), true);
    ...
    pool.addUnchecked(entry.Fee(1LL).Time(2).FromTx(tx8), setAncestors);
    ...
    pool.addUnchecked(entry.Fee(1LL).Time(3).FromTx(tx9), setAncestors);
    ...
    BOOST_CHECK_EQUAL(pool.CalculateMemPoolAncestors(entry.Fee(200000LL).Time(4).FromTx(tx10), setAncestorsCalculated, 100, 1000000, 1000, 1000000, dummy, false, nNoLimit, nNoLimit, zero, zero), true);
    BOOST_CHECK(setAncestorsCalculated == setAncestors);
