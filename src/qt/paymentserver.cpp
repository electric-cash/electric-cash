...
const int ELCASH_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString ELCASH_IPC_PREFIX("bitcoin:");
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(ELCASH_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(ELCASH_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(ELCASH_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    else if (s.startsWith(ELCASH_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
