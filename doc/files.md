...
`./`               | `bitcoin.conf`        | Contains [configuration settings](bitcoin-conf.md) for `elcashd` or `bitcoin-qt`; can be specified by `-conf` option
`./`               | `elcashd.pid`        | Stores the process ID (PID) of `elcashd` or `bitcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `elcashd` or `bitcoin-qt`; can be specified by `-debuglogfile` option
